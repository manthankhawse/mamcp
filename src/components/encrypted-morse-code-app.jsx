...
export function EncryptedMorseCodeAppComponent() {
  ...
  const [history, setHistory] = useState([]);
  const [encryptionKey, setEncryptionKey] = useState("SECRET");
  const [isCorrectKey, setIsCorrectKey] = useState(false); // State to track key validity
  ...
  const validateKey = () => {
    if (!encryptionKey) {
      setIsCorrectKey(false); // Key is empty, set validity to false
      return;
    }
    
    const decrypted = decrypt(receivedMessage, encryptionKey);
    setIsCorrectKey(decrypted === receivedMessage); // Update based on comparison
  };
    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-4">
      <Card className="w-full max-w-4xl bg-gray-800 border-gray-700">
        ...
        <CardContent>
          <div className="space-y-6">
            {/* <div className="flex items-center space-x-2">
              <Label htmlFor="encryption-key" className="text-white">
                Encryption Key:
              </Label>
              <Input
                id="encryption-key"
